...
import { handleTradeInteraction, handleSeashellsModalSubmit } from "../../handlers/tradeHandler.js";
import { handleSellFishSelect, handleSellButtons } from "../../handlers/sellHandler.js";
...
export default class InteractionCreate extends Event {
    ...
    async run(interaction) {
        ...
        if (interaction.type === InteractionType.ModalSubmit) {
            // Check if it's a trade seashells modal
            if (interaction.customId.startsWith('trade_seashells_modal_')) {
                return handleSeashellsModalSubmit(interaction, this.client);
            }
        }
        
        // Handle button interactions (for trading system)
        if (interaction.type === InteractionType.MessageComponent && interaction.isButton()) {
            // Check if it's a trade button
            if (interaction.customId.startsWith('trade_')) {
                return handleTradeInteraction(interaction, this.client);
            }
            
            // Check if it's a sell button
            if (interaction.customId.startsWith('sell_all_common_') || interaction.customId.startsWith('sell_all_')) {
                return handleSellButtons(interaction, this.client);
            }
        }
        
        // Handle String Select Menu interactions
        if (interaction.type === InteractionType.MessageComponent && interaction.isStringSelectMenu()) {
            // Check if it's a trade select menu
            if (interaction.customId.startsWith('trade_')) {
                return handleTradeInteraction(interaction, this.client);
            }
            
            // Check if it's a sell fish select menu
            if (interaction.customId.startsWith('sell_fish_select_')) {
                return handleSellFishSelect(interaction, this.client);
            }
        }
